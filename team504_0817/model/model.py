...
parser.add_argument("--test_data", type=str, default="../test/", help="test set")
parser.add_argument(
    "--save_result", type=bool, default=True, help="save the reconstruction or not"
)
parser.add_argument(
    "--result_dir", type=str, default="../result_img/", help="path of results"
)
parser.add_argument(
    "--result_sr_dir", type=str, default="../result_sr_img/", help="path of results"
)
parser.add_argument(
    "--exist_gt", type=bool, default=True, help="exist ground truth or not"
)
parser.add_argument("--model_name", type=str, default="model_062.pth", help="Name of ckp")
...
#            out_rec = out_rec[:250, :]
            ...
            if opt.save_result:
                print(os.path.join(opt.result_dir, "00"+ str(i)+".png"))
                cv2.imwrite(
                    os.path.join(opt.result_dir, "00"+str(i)+".png"), out_rec
                )
                sr_rec = upsample(out_rec)
                print(np.shape(sr_rec))
                cv2.imwrite(
                    os.path.join(opt.result_sr_dir, "00"+str(i)+ ".png"), sr_rec
                )
